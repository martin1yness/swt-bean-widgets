...
public class WindowSWTBotTest {
            ...
            @Override public void run() {
                shell = new Window() {
                    ...
                    @Override protected void preInitGUI() {
                        applyTitleTagPostfix(" [POSTFIX TITLE]");
                    ...
                    @Override protected void applyColorSchemeOverrides(ColorScheme scheme) {
    ...
    @Test public void colorSchemeApplied() throws Exception {
        ...
        Assert.assertEquals(123, button.backgroundColor().getGreen());
        Assert.assertEquals(213, button.foregroundColor().getGreen());
        Display.getDefault().syncExec(new Runnable() {
            public void run() {
                Assert.assertEquals(null, button.widget.getBackgroundImage());
            }
        });

        // Rule 1 applies
        final SWTBotLabel label = bot.label("Label A:");
        Assert.assertEquals(0, label.backgroundColor().getBlue());
        Assert.assertEquals(255, label.foregroundColor().getBlue());
        Display.getDefault().syncExec(new Runnable() {
            public void run() {
                Assert.assertEquals(null, label.widget.getBackgroundImage());
            }
        });
        ...
        Display.getDefault().syncExec(new Runnable() {
            public void run() {
                Assert.assertEquals(null, text.widget.getBackgroundImage());
            }
        });
    ...
    @Test public void testPostfixPrefixTitleApplies() {
    ...
    @Test public void testCloseAllWindowsFeature() {
        Display.getDefault().syncExec(new Runnable() {
            public void run() {
                shell2.open();
        ...
        /*
        SWTBotShell botShell = bot.shell("Fail Close [POSTFIX TITLE]");
        assertNotNull(shell);
        assertTrue(botShell.isOpen());
        Display.getDefault().syncExec(new Runnable() {
            public void run() {
                assertTrue(Window.closeAll());
            }
        });
        assertFalse(botShell.isOpen());
        */
    }
}
